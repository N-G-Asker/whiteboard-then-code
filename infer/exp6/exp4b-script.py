...
def create_prompt_for_feedback(sample, original_plan):
    ...
    system = "You are an algorithms expert focused on correctness and efficiency, including time complexity and space complexity."
...
def make_generations(dataset, args, model, tokenizer):
    ...
    for task in tqdm(range(n_tasks)):
        ...
        if round > 0 and round % 50 == 0:
            filename = f"b4_feedback_up_to_iter_{round}__"
    ...
    filename = f"b4_feedback_all_{round}_iters_"
    filename += strftime("%d_%b_%Y_%I.%M%p", localtime())
    with open(filename, "wb") as gen_dump:
        pickle.dump(feedback, gen_dump)
    ...
    for task in tqdm(range(n_tasks)):
        ...
        # Periodically save generations to a file
        if round > 0 and round % 50 == 0:
            filename = f"b4_rewrites_up_to_iter_{round}__"
            filename += strftime("%d_%b_%Y_%I.%M%p", localtime())
            with open(filename, "wb") as gen_dump:
    ...
    # Save generations to a file
    filename = f"b4_rewrites_all_{round}_iters_"
    filename += strftime("%d_%b_%Y_%I.%M%p", localtime())
    with open(filename, "wb") as gen_dump:
        pickle.dump(rewrites, gen_dump)
    ###############################################################
    ...
    for task in tqdm(range(n_tasks)):
        ...
        # Periodically save generations to a file
        if round > 0 and round % 50 == 0:
            filename = f"b4_generations_up_to_iter_{round}__"
            filename += strftime("%d_%b_%Y_%I.%M%p", localtime())
            with open(filename, "wb") as gen_dump:
                pickle.dump(generations, gen_dump)

            filename = f"b4_code_prompts_up_to_iter_{round}__"
            filename += strftime("%d_%b_%Y_%I.%M%p", localtime())
    ...
    # Save generations to a file
    filename = f"b4_generations_all_{round}_iters_"
    filename += strftime("%d_%b_%Y_%I.%M%p", localtime())
    with open(filename, "wb") as gen_dump:
        pickle.dump(generations, gen_dump)

    # Save code-prompts to a file
    filename = f"b4_code_prompts_all_{round}_iters_"
    filename += strftime("%d_%b_%Y_%I.%M%p", localtime())
    with open(filename, "wb") as prompt_dump:
        pickle.dump(prompts, prompt_dump)
