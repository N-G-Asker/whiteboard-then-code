...
def create_prompt_for_content_plan(sample):
    ...
    _input = "[INST] Briefly explain how to solve the following problem. "
    _input += "Don't code, just describe in words a solution that "
    _input += "makes sense and answers the problem:\n"
...
def make_generations(dataset, args, model, tokenizer):
    ...
    for task in tqdm(range(n_tasks)):
        ...
        if round > 0 and round % 50 == 0:
            ...
            filename = f"b2_code_prompts_up_to_iter_{round}__"
            filename += strftime("%d_%b_%Y_%I.%M%p", localtime())
    ...
    filename = f"b2_code_prompts_all_{round}_iters_"
    filename += strftime("%d_%b_%Y_%I.%M%p", localtime())
    with open(filename, "wb") as prompt_dump:
        pickle.dump(prompts, prompt_dump)
