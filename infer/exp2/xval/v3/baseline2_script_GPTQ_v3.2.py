...
def create_prompt_for_content_plan(sample):
    ...
    _input += "Don't code, just describe in words a high-level solution that "
    _input += "makes sense and answers the problem. Provide enough detail so "
    _input += "someone else can implement the solution later:\n"
    _input += f"{sample['question']}\n"
    # _input += f"{starter_code_text}" # exclude starter cofr from prompt for plans
    _input += "[/INST]"
...
def create_prompt_for_code(sample, content_plan):
    starter_code_text = "" if len(sample["starter_code"]) == 0 else sample["starter_code"] 
    try:
        input_outpout = json.loads(sample["input_output"])
        fn_name = None if not input_outpout.get("fn_name") else input_outpout["fn_name"] 
    except ValueError:
...
def complete_code(pipe, prompt, starter_code_len, num_completions=1, **code_gen_kwargs):
    ...
    try:
        ...
        return [code_gen["generated_text"][(len(prompt) - starter_code_len):] for code_gen in code_gens]
    except IndexError:
...
def make_generations(dataset, args, model, tokenizer):
    ...
    for task in tqdm(range(n_tasks)):
        task_generations = []
        sample = dataset[task]
        prompt = create_prompt_for_code(sample, plans[round]).strip()
        starter_code_text = "" if len(sample["starter_code"]) == 0 else sample["starter_code"] 
        task_generations.extend(complete_code(pipe, prompt, len(starter_code_text), num_completions=args.n_samples, **code_gen_kwargs))
