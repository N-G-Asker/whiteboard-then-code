...
def create_prompt_for_feedback(sample, original_plan):
    ...
    """    
    _input = "[INST] Review my approach to solving the following problem:\n"
    _input += f"{sample['question']}\n"
    _input += "My approach:\n"
    _input += f"{original_plan}\n"
    _input += "Give one suggestion to help improve my approach. " # Idea for this specific feedback prompt comes from the Self-Refine paper (https://arxiv.org/pdf/2303.17651.pdf). See Figure 22: FEEDBACK prompt for Code Readability
    _input += "Respond in five sentences or fewer.\n" # Idea for this stop-generation method comes from a blog post by Shuyang Xiang: https://towardsdatascience.com/challenges-in-stop-generation-within-llama-2-25f5fea8dea2
    _input += "[/INST]"
...
def create_prompt_for_revised_plan(sample, original_plan, feedback_text):
    ...
    Args:
        ...
        plan: the initial content plan, on which we are soliciting the generator
              for feedback
    """
    _input = "[INST] Rewrite my original approach "
    _input += "to solving the following problem:\n"
    _input += f"{sample['question']}\n"
    _input += f"Original approach:\n"
    _input += f"{original_plan}\n"
    _input += "Feedback:\n"
    _input += f"{feedback_text}\n"
    _input += "Fix my original approach by incorporating the feedback. "
    _input += "Respond in twelve sentences or fewer.\n"
    _input += "[/INST] Sure! Here is the rewrite:\n"
    return _input
